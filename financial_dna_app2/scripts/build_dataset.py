...
SECTOR_COMPANIES = {
    ...
    "Industrials": [
        ...
        # Airlines
        ("DAL", "Delta Air Lines", "Airlines"), ("UAL", "United Airlines", "Airlines"),
        ("AAL", "American Airlines", "Airlines"), ("LUV", "Southwest Airlines", "Airlines"),
        ("ALK", "Alaska Air Group", "Airlines"), ("JBLU", "JetBlue Airways", "Airlines"),
        # Trucking / rental
        ("ODFL", "Old Dominion Freight Line", "Trucking"), ("JBHT", "JB Hunt Transport", "Trucking"),
        ("XPO", "XPO Inc", "Trucking"), ("KNX", "Knight-Swift Transportation", "Trucking"),
        ("URI", "United Rentals", "Industrial Equipment Rental"),
        # Professional services / waste
        ("RSG", "Republic Services", "Waste Management"), ("VRSK", "Verisk Analytics", "Research & Consulting"),
        ("AXON", "Axon Enterprise", "Aerospace & Defense"), ("HEI", "HEICO Corp", "Aerospace & Defense"),
...
def _draw_company_ratios(rng, profile):
    """
    Draw one consistent ratio vector for a company.

    Enforces the accounting identities that the original independent-normal
    sampler was violating:
      - Gross_Margin = 1 - COGS_to_Revenue (by definition)
      - Cash, A/R, Inventory, PP&E are non-negative (percent of assets)
      - Debt/Assets + Equity/Assets <= 1 (other liabilities make up the slack)
    """
    # Income statement: draw COGS, derive Gross Margin
    cogs = float(np.clip(rng.normal(*profile["COGS_to_Revenue"]), 0.01, 0.95))
    gross_margin = 1.0 - cogs  # identity
    sga = float(np.clip(rng.normal(*profile["SGA_to_Revenue"]), 0.0, 0.80))
    net_margin = float(np.clip(rng.normal(*profile["Net_Margin"]), -0.30, 0.60))

    # Balance-sheet composition: force percentages >= 0
    cash = float(np.clip(rng.normal(*profile["Cash_to_Assets"]), 0.0, 0.60))
    ar = float(np.clip(rng.normal(*profile["Receivables_to_Assets"]), 0.0, 0.40))
    inv = float(np.clip(rng.normal(*profile["Inventory_to_Assets"]), 0.0, 0.35))
    ppe = float(np.clip(rng.normal(*profile["PPE_to_Assets"]), 0.0, 0.85))

    # Equity first, then cap debt so the identity holds with a small cushion
    equity = float(np.clip(rng.normal(*profile["Equity_to_Assets"]), 0.02, 0.95))
    max_debt = max(0.05, 1.0 - equity - 0.02)  # leave >=2% for other liabilities
    debt = float(np.clip(rng.normal(*profile["Debt_to_Assets"]), 0.05, max_debt))

    return {
        "COGS_to_Revenue": round(cogs, 4),
        "Gross_Margin": round(gross_margin, 4),
        "SGA_to_Revenue": round(sga, 4),
        "Net_Margin": round(net_margin, 4),
        "Cash_to_Assets": round(cash, 4),
        "Receivables_to_Assets": round(ar, 4),
        "Inventory_to_Assets": round(inv, 4),
        "PPE_to_Assets": round(ppe, 4),
        "Debt_to_Assets": round(debt, 4),
        "Equity_to_Assets": round(equity, 4),
    }


def generate_synthetic_dataset(seed=42, samples_per_company=5):
    """
    Generate a consistent synthetic dataset based on known industry financial profiles.

    Each real S&P 500 company is used as a "seed" for `samples_per_company` synthetic
    ratio vectors drawn from that sector's distribution, with accounting identities
    enforced.  This gives the model more training signal without manufacturing fake
    tickers.  The first sample per company keeps the original ticker; additional
    samples are tagged with a -v2, -v3, ... suffix for traceability.
    """
    ...
    for sector, companies in SECTOR_COMPANIES.items():
        ...
        for ticker, name, sub_industry in companies:
            for rep in range(samples_per_company):
                record = {
                    "ticker": ticker if rep == 0 else f"{ticker}-v{rep + 1}",
                    "company_name": name,
                    "sector": sector,
                    "industry": sub_industry,
                    "is_synthetic_variant": rep > 0,
                }
                record.update(_draw_company_ratios(rng, profile))
...
def build_dataset(force_synthetic=False, samples_per_company=5):
    print("=" * 60)
    ...
    if not force_synthetic and try_live_build():
        ...
        print("(This will take ~10-20 minutes for 500+ companies; resumable)")

        from utils.data_fetcher import (
            get_sp500_tickers, fetch_company_data, fetch_company_data_multi_year,
        )

        sp500 = get_sp500_tickers()

        # If the Wikipedia scrape failed (fallback returns ~72 tickers) fall back
        # to our curated SECTOR_COMPANIES list, which already has ~300 tickers
        # with proper GICS sector/sub-industry tags.
        if len(sp500) < 200:
            print(f"  Wikipedia scrape returned only {len(sp500)} tickers — "
                  "using the built-in SECTOR_COMPANIES list (~300 tickers) instead.")
            rows = []
            for sector, companies in SECTOR_COMPANIES.items():
                for ticker, name, sub_industry in companies:
                    rows.append({
            ...
            sp500 = pd.DataFrame(rows)

        # Resume support: if a partial CSV already exists, keep its rows and skip
        # tickers that already have any row present so a rate-limited run can
        # restart without starting over.
        partial_path = os.path.join(DATA_DIR, "company_ratios.csv")
        existing = None
        if os.path.exists(partial_path):
            try:
                existing = pd.read_csv(partial_path)
                have_bases = set(existing["ticker"].astype(str).str.upper())
                sp500 = sp500[~sp500["Symbol"].str.upper().isin(have_bases)].reset_index(drop=True)
                print(f"  Found {len(have_bases)} tickers already in {partial_path}; "
                      f"resuming with {len(sp500)} remaining.")
            except Exception:
                existing = None
        ...
        for i, row in sp500.iterrows():
            ...
            gics_industry = row.get("GICS Sub-Industry")
            name = row.get("Security", ticker)

            if (i + 1) % 10 == 0:
                print(f"  {i + 1}/{len(sp500)}: {ticker}  "
                      f"(companies ok: {ok_companies}, rows so far: {len(records)}, "
                      f"failures: {fail_count})")

            # Multi-year pull: up to 5 fiscal years per company, so one good
            # ticker can contribute 3-5 rows to the training set.
            rows_for_ticker = fetch_company_data_multi_year(
                ticker,
                preferred_sector=gics_sector,
                preferred_industry=gics_industry,
                preferred_name=name,
                max_years=5,
            )
            if rows_for_ticker:
                records.extend(rows_for_ticker)
                ok_companies += 1
            else:
                fail_count += 1

            # Gentle pacing: 0.3s between tickers, longer pause every 25 to dodge
            # Yahoo's rate limiter.  Total run time ≈ (N * 0.3s) + (N/25 * 3s).
            time.sleep(0.3)
            if (i + 1) % 25 == 0:
                time.sleep(3)

            # Flush progress every 50 tickers so interrupted runs can resume.
            if (i + 1) % 50 == 0 and records:
        ...
        print(f"\n  Live fetch complete: {ok_companies} companies ({len(df)} rows total), "
              f"{fail_count} failed tickers.")

        # Live data has gaps and outliers — impute with sector median and clip
        # 3-sigma outliers per column.
        ...
        print(f"\nUsing synthetic data based on real industry financial profiles "
              f"({samples_per_company}x samples per company).")
        print("(Run on your local machine with internet to use live yfinance data)")
        df = generate_synthetic_dataset(samples_per_company=samples_per_company)
        # Synthetic data is already identity-consistent, no further cleanup required.
...
if __name__ == "__main__":
    ...
    # Optional: --samples-per-company N
    samples = 5
    for i, arg in enumerate(sys.argv):
        if arg == "--samples-per-company" and i + 1 < len(sys.argv):
            samples = int(sys.argv[i + 1])
    build_dataset(force_synthetic=force, samples_per_company=samples)
