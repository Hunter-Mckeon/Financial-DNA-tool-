...
with col_input2:
    # Build options dynamically based on what was loaded
    _opts = ["Random Forest", "XGBoost", "Logistic Regression"]
    if "svc" in models:
        _opts.append("SVC (rbf)")
    if "ensemble" in models:
        _opts.append("Voting Ensemble")
    model_choice = st.selectbox("Select ML Model", options=_opts, index=0)
...
if run_button and ticker_input:
    ...
    # ── Ensemble transparency: show each model's vote when the user picks
    # "Voting Ensemble". The trained VotingClassifier uses all 5 models
    # (RF + XGB + LR + KNN + SVC), so every row is a real voter and 5 voters
    # means plurality ties are impossible. ──
    if model_key == "ensemble":
        ...
        if "svc" in models:
            ensemble_members.append(("svc", "SVC (rbf)"))

        vote_rows = []
        for key, label in ensemble_members:
            ind_pred, ind_probs = predict_industry(ratios, models[key], scaler, le)
            ind_conf = ind_probs.get(ind_pred, 0.0) if ind_probs else 0.0
            vote_rows.append({
                "Model": label,
                "Predicted Sector": ind_pred,
                "Confidence": f"{ind_conf:.1%}" if ind_conf else "n/a",
                "Agrees With Ensemble?": "✅" if ind_pred == predicted_sector else "❌",
            })

        votes_df = pd.DataFrame(vote_rows)
        st.dataframe(votes_df, hide_index=True, use_container_width=True)

        # Tally votes across all ensemble members
        from collections import Counter
        ensemble_preds = [r["Predicted Sector"] for r in vote_rows]
        tally = Counter(ensemble_preds)
        top_sector, top_count = tally.most_common(1)[0]
        n_voters = len(ensemble_preds)

        plurality_line = f"Plurality vote: **{top_sector}** ({top_count}/{n_voters} models)."

        if top_sector == predicted_sector:
            agree_line = "Soft-voting ensemble agreed."
        else:
            agree_line = (
                f"Soft-voting ensemble chose **{predicted_sector}** — it averages each "
                "model's full probability distribution, so a very confident vote "
                "(like XGBoost at 90%+) can outweigh multiple less-confident votes."
            )

        st.caption(plurality_line + " " + agree_line)
